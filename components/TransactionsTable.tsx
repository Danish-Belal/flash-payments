  ...
  return (
     ...
     <TableBody>
          {transactions.map((t: Transaction)=>{
               ...
               const isCradit = t.type === 'credit';

               return(
                    <TableRow key={t.id} className={`${isDebit || amount[0] == '-' ? 'bg-[#FFFBFA]' : 'bg[#F6FEF9]' } !over:bg-nono !border-b-DEFAULT`}>
                         ...
                         <TableCell className={`pl-2 pr-10 font-semibold ${isDebit || amount[0] == '-' ? 'text-[#F04438]' : 'text-[#039855]'}`} >
                              {isDebit ? `-${amount}` : isCradit ? amount: amount}
                         </TableCell>
                         <TableCell className='pl-2 pr-10'> <CategoryBatch category={status} /></TableCell>
                         ...
                         <TableCell className='pl-2 pr-10 max-md:hidden'><CategoryBatch category={t.category} /></TableCell>
                    </TableRow>
               )
          })}
